...
@respx.mock
async def test_delete_sequence(client: Orch8Client) -> None:
    respx.delete(f"{BASE}/sequences/seq-1").mock(
        return_value=httpx.Response(204)
    )
...
@respx.mock
async def test_migrate_instance(client: Orch8Client) -> None:
    respx.post(f"{BASE}/sequences/migrate-instance").mock(
        return_value=httpx.Response(200, json=INSTANCE_JSON)
    )
    result = await client.migrate_instance(
        {"instance_id": "inst-1", "target_sequence_id": "seq-2"}
    )
    assert isinstance(result, TaskInstance)
    assert result.id == "inst-1"
...
@respx.mock
async def test_inject_blocks(client: Orch8Client) -> None:
    respx.post(f"{BASE}/instances/inst-1/inject-blocks").mock(
        return_value=httpx.Response(204)
    )
    result = await client.inject_blocks(
        "inst-1", [{"type": "task", "handler": "my-handler"}]
    )
    assert result is None
...
@respx.mock
async def test_list_worker_tasks(client: Orch8Client) -> None:
    respx.get(f"{BASE}/workers/tasks").mock(
        return_value=httpx.Response(200, json=[WORKER_TASK_JSON])
    )
    result = await client.list_worker_tasks()
    assert isinstance(result, list)
    assert len(result) == 1
    assert isinstance(result[0], WorkerTask)
    assert result[0].id == "wt-1"
...
@respx.mock
async def test_get_worker_task_stats(client: Orch8Client) -> None:
    stats_json = {"pending": 5, "running": 2, "completed": 100}
    respx.get(f"{BASE}/workers/tasks/stats").mock(
        return_value=httpx.Response(200, json=stats_json)
    )
    result = await client.get_worker_task_stats()
    assert isinstance(result, dict)
...
@respx.mock
async def test_poll_tasks_from_queue(client: Orch8Client) -> None:
    respx.post(f"{BASE}/workers/tasks/poll/queue").mock(
        return_value=httpx.Response(200, json=[WORKER_TASK_JSON])
    )
...
@respx.mock
async def test_list_pools(client: Orch8Client) -> None:
    respx.get(f"{BASE}/pools").mock(
        return_value=httpx.Response(200, json=[POOL_JSON])
    )
    result = await client.list_pools()
    assert isinstance(result, list)
    assert len(result) == 1
    assert isinstance(result[0], ResourcePool)
    assert result[0].id == "pool-1"
    assert result[0].name == "gpu-pool"


@respx.mock
async def test_create_pool(client: Orch8Client) -> None:
    respx.post(f"{BASE}/pools").mock(
        return_value=httpx.Response(201, json=POOL_JSON)
    )
    result = await client.create_pool({"name": "gpu-pool", "max_size": 10})
...
@respx.mock
async def test_get_pool(client: Orch8Client) -> None:
    respx.get(f"{BASE}/pools/pool-1").mock(
        return_value=httpx.Response(200, json=POOL_JSON)
    )
    result = await client.get_pool("pool-1")
    assert isinstance(result, ResourcePool)
    assert result.id == "pool-1"
...
@respx.mock
async def test_delete_pool(client: Orch8Client) -> None:
    respx.delete(f"{BASE}/pools/pool-1").mock(
        return_value=httpx.Response(204)
    )
    result = await client.delete_pool("pool-1")
    assert result is None


@respx.mock
async def test_list_pool_resources(client: Orch8Client) -> None:
    respx.get(f"{BASE}/pools/pool-1/resources").mock(
        return_value=httpx.Response(200, json=[RESOURCE_JSON])
    )
    result = await client.list_pool_resources("pool-1")
    assert isinstance(result, list)
    assert len(result) == 1
    assert isinstance(result[0], PoolResource)
    assert result[0].id == "res-1"
    assert result[0].resource_key == "gpu-0"


@respx.mock
async def test_create_pool_resource(client: Orch8Client) -> None:
    respx.post(f"{BASE}/pools/pool-1/resources").mock(
        return_value=httpx.Response(201, json=RESOURCE_JSON)
    )
    result = await client.create_pool_resource(
        "pool-1", {"resource_key": "gpu-0"}
    )
    assert isinstance(result, PoolResource)
    assert result.pool_id == "pool-1"
    assert result.state == "available"


@respx.mock
async def test_update_pool_resource(client: Orch8Client) -> None:
    updated = {**RESOURCE_JSON, "state": "locked"}
    respx.put(f"{BASE}/pools/pool-1/resources/res-1").mock(
        return_value=httpx.Response(200, json=updated)
    )
    result = await client.update_pool_resource(
        "pool-1", "res-1", {"state": "locked"}
    )
    assert isinstance(result, PoolResource)
...
@respx.mock
async def test_delete_pool_resource(client: Orch8Client) -> None:
    respx.delete(f"{BASE}/pools/pool-1/resources/res-1").mock(
        return_value=httpx.Response(204)
    )
    result = await client.delete_pool_resource("pool-1", "res-1")
    assert result is None
...
@respx.mock
async def test_list_credentials(client: Orch8Client) -> None:
    respx.get(f"{BASE}/credentials").mock(
        return_value=httpx.Response(200, json=[CREDENTIAL_JSON])
    )
...
@respx.mock
async def test_create_credential(client: Orch8Client) -> None:
    respx.post(f"{BASE}/credentials").mock(
        return_value=httpx.Response(201, json=CREDENTIAL_JSON)
    )
    result = await client.create_credential(
        {"name": "api-key", "credential_type": "api_key", "value": "secret"}
    )
...
@respx.mock
async def test_get_credential(client: Orch8Client) -> None:
    respx.get(f"{BASE}/credentials/cred-1").mock(
        return_value=httpx.Response(200, json=CREDENTIAL_JSON)
    )
...
@respx.mock
async def test_delete_credential(client: Orch8Client) -> None:
    respx.delete(f"{BASE}/credentials/cred-1").mock(
        return_value=httpx.Response(204)
    )
...
@respx.mock
async def test_update_credential(client: Orch8Client) -> None:
    updated = {**CREDENTIAL_JSON, "name": "updated-key"}
    respx.patch(f"{BASE}/credentials/cred-1").mock(
        return_value=httpx.Response(200, json=updated)
    )
    result = await client.update_credential("cred-1", {"name": "updated-key"})
    assert isinstance(result, Credential)
...
@respx.mock
async def test_list_tenant_circuit_breakers(client: Orch8Client) -> None:
    respx.get(f"{BASE}/tenants/t-1/circuit-breakers").mock(
        return_value=httpx.Response(200, json=[CIRCUIT_BREAKER_JSON])
    )
    result = await client.list_tenant_circuit_breakers("t-1")
    assert isinstance(result, list)
    assert len(result) == 1
    assert isinstance(result[0], CircuitBreaker)
    assert result[0].handler == "my-handler"
...
@respx.mock
async def test_get_tenant_circuit_breaker(client: Orch8Client) -> None:
    respx.get(f"{BASE}/tenants/t-1/circuit-breakers/my-handler").mock(
        return_value=httpx.Response(200, json=CIRCUIT_BREAKER_JSON)
    )
    result = await client.get_tenant_circuit_breaker("t-1", "my-handler")
    assert isinstance(result, CircuitBreaker)
    assert result.state == "closed"


@respx.mock
async def test_reset_tenant_circuit_breaker(client: Orch8Client) -> None:
    respx.post(f"{BASE}/tenants/t-1/circuit-breakers/my-handler/reset").mock(
        return_value=httpx.Response(204)
    )
